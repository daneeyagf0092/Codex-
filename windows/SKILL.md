...
description: Apply, launch, verify, repair, update, or restore a full decorative skin for the Windows Codex desktop app. Use when the user asks for a Codex theme that goes beyond official color settings, wants the cream/mint Inspiration Micro-Universe interface, needs the skin reapplied after a Codex update, or needs a safe rollback without modifying WindowsApps or app.asar.
---
...
1. Run `scripts/install-dream-skin.ps1` once to set the matching cream/mint official base colors and create launch/restore shortcuts.
...
- Do not use the gallery reference screenshot as a fake whole-window overlay or crop it into the live hero; all controls and visible panels remain live Codex controls.
- Keep the home decoration CSS-native unless a clean, dedicated banner asset is provided. Never crop a gallery preview composite into the live interface. Keep the cards below it as native Codex suggestion buttons with native labels/icons.
...
- `docs/images/gallery/skin-05.jpg`: visual direction reference only; never loaded into the live interface.
